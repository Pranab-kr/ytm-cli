...
pub enum PlayerCommand {
    ...
    /// Play the entry already at this index (FR-Q1). Distinct from `PlayNow`, which
    /// *inserts*: Enter on a queue row went through `PlayNow`, so replaying a
    /// finished track pushed a second copy in beside the first.
    JumpTo(usize),
    MoveInQueue {
        from: usize,
        to: usize,
